...
public sealed partial class ViewLaterPage : Page, IMainPanelPage
{
    ...
    public Task ActivateAsync()
    {
        return _session is not null
            ? _session.RefreshViewLaterAsync()
            : Task.CompletedTask;
    }

    private void OnViewLaterRefreshed(object? sender, EventArgs e)
    {
        DispatcherQueue.TryEnqueue(RenderViewLaterCards);
    }
    ...
    private async void ViewLaterScrollViewer_ViewChanged(object? sender, ScrollViewerViewChangedEventArgs e)
    {
        ...
        if (distanceToBottom <= 40 && _session is not null)
            await _session.LoadMoreViewLaterAsync();
    }
