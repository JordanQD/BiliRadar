...
public sealed partial class HistoryPage : Page, IMainPanelPage
{
    ...
    public Task ActivateAsync()
    {
        return _session is not null
            ? _session.RefreshHistoryAsync()
            : Task.CompletedTask;
    }

    private void OnHistoryRefreshed(object? sender, EventArgs e)
    {
        DispatcherQueue.TryEnqueue(RenderHistoryCards);
    }
    ...
    private async void HistoryScrollViewer_ViewChanged(object? sender, ScrollViewerViewChangedEventArgs e)
    {
        ...
        if (distanceToBottom <= 40 && _session is not null)
            await _session.LoadMoreHistoryAsync();
    }
