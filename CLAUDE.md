...
以下 Phase 1–6 为旧 WinUIEx/页面迁移历史记录；当前 DesktopFlyouts 迁移的实施状态以 `docs/design/desktopflyouts-migration.md` 为准。

| 阶段 | 状态 |
|------|------|
| Phase 1 (WinUIEx 原型，历史) | ✅ |
| Phase 2a-2f (页面提取) | ✅ |
| Phase 2g (Flyout 集成) | ✅ 已手动验证 |
| Phase 3 (右键菜单迁移) | ✅ 新路径原生 MenuFlyout，仅保留设置/退出 |
| Phase 4 (清理旧代码，历史) | ✅ 删除 MainWindow、TrayIconService、SystemTray/，#if 双路径已移除 |
...
详见 `docs/design/desktopflyouts-migration.md`；旧 WinUIEx 迁移记录见 `docs/design/tray-flyout-migration.md`。
...
当前分支 `codex/desktopflyouts-migration`，主分支 `main`。
